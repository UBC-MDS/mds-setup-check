...
The fixtures are organised by **input format**, because that is the choice you make
first and cannot easily undo: two `.qmd` (one per language), one `.ipynb`, one
`.Rmd`. All four contain the same features, so any difference between them below is
a property of the toolchain rather than of the document.

`check-notebook-table.ipynb` is the one exception, and the reason is worth knowing:
its single construct does not merely render wrong, it stops the render dead. A
construct like that takes every other construct in the file down with it, so it has
to sit alone. Everything that fails *quietly* stays in the four full fixtures, where
its own column does the attributing. A dash means the fixture does not contain that
feature at all, which is a different statement from a cross.

Read the **Mathematics** section of any full fixture if you write assignments. It
carries every equation form you are likely to use, each labelled with the routes that
keep it -- and one rule explains the whole table: if it is inside `$` or `$$`, pandoc
parses it and every route sets it; if it is not, pandoc forwards it to LaTeX and the
other writers discard it silently.
